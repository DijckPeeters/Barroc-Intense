    ...
    public sealed partial class FormulierPage : Page
    {
        ...
        private async void Opslaan_Click(object sender, RoutedEventArgs e)
        {
            if (_melding.IsKeuring)
            {
                ...
                // Voorraad aftrekken bij goedgekeurde keuring (1x)
                if (_melding.KeuringGoedgekeurd && !_melding.IsKeuringVoltooid)
                {
                    // Haal alles eerst in memory om MySQL connectieprobleem te voorkomen
                    var materialen = _context.MaintenanceMaterials
                        .Where(x => x.MeldingId == _keuringId)
                        .ToList(); // Nu in memory

                    var alleMaterials = _context.Materials.ToList(); // Alles in memory
            ...
            // Alles opslaan in één SaveChangesAsync call
            await _context.SaveChangesAsync();
